...
if($datep) {
	$query = $bdd->query("TRUNCATE TABLE date_resultat");
	$datepcontent = substr(fgets($datep), 1, -1);
	if($datepcontent != "") {
		$buffer = explode(", ", $datepcontent);
		for ($i = 0; $i < count($buffer); $i++) {
			$query = $bdd->query("INSERT INTO date_resultat (date) VALUES (".$buffer[$i].")");
		}	
	}
}
...
if ($fp)
{
	while (!feof($fp))
	{
		...
		if($j != NULL) {
			...
			if($row = $query->fetch()) {
				if((($j->{'score'} != " ?") and (($row['score'] == " ?") or ($row['score'] == "Report&eacute;") or ($row['score'] == "Report&eacute")  or ($row['score'] == "Non jou&eacute;"))) or (($j->{'score'} == " ?") and (($row['score'] == 'Report&eacute') or ($row['score'] == 'Report&eacute;')))) {
