...
def parse_coupe(id, slug, phase, poule):
    ...
    list_dates = famfoot.previous_week_end()
    for date in list_dates:
        listdates.append(famfoot.retrieve_date(html, date, 1))

    for date in listdates:
        if(date != ""):
            list_matchs = famfoot.gather_matchs(html, date, link, 1, 1) 
            for match in list_matchs:



def parse_champ(id, slug, phase, poule):
    link = "https://www.fff.fr/championnats/fff/national/" + str(id) + "-" + slug + "/phase-" + str(phase) + "/poule-" + str(poule) + "/derniers-resultats?showForm=true&use_automatic_dates=1&type=full"
    req = urllib.request.Request(link, headers={'User-Agent': 'Mozilla/5.0 (Macintosh Intel Mac OS X 10_9_3) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/35.0.1916.47 Safari/537.36'})
    response = urllib.request.urlopen(req)
    html = str(response.read().decode('utf-8'))
    list_dates = famfoot.previous_week_end()
